...
## Part A — The Bill of Rights (18 Rules)

### Right 1: One Writer Per Global
**Every `window.*` / shared-state entrypoint has exactly ONE canonical writer module.**

- Direct `window.X = ...` assignments outside the canonical writer are banned.
- Readers may access `window.X` freely, but must never mutate it.
- If a second module needs to set the value, it must call the canonical writer's exported function.
- Enforcement: `grep -rn 'window\.LuxFoo\s*=' --include='*.js'` must return exactly one file.
...
2. **Create `.GOLD` backup**: `cp file.js file.js.GOLD`
