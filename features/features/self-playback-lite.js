...
// DISABLED (parking file): suspected dead/bloat.
// Goal: keep file in repo but guarantee it does nothing at runtime.
//
// How to re-enable:
// 1) Remove the early return below, OR flip the flag to true.
// 2) Restore the real exports (or uncomment the original code block).

const LUX_SELF_PB_LITE_ENABLED = false;

export function mountSelfPlaybackLite() {
  // Stub export so any import doesn't crash.
  // When disabled, do nothing.
  return;
}

if (!LUX_SELF_PB_LITE_ENABLED) {
  // Hard stop. Nothing below executes.
  // (Keeping this file inert prevents double-mount/ticker issues and avoids hidden side-effects.)
} else {
  // If you ever re-enable, you'd move the original code back out of comments and restore exports.
}

/* ========================================================================== */
/* ORIGINAL FILE (COMMENTED OUT / PARKED)                                     */
/* ========================================================================== */
