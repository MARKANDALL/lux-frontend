...
export function stopMic(state) {
  ...
  if (state.mic.audioCtx) {
    try { state.mic.audioCtx.close(); } catch (err) { globalThis.warnSwallow("features/onboarding/onboarding-mic.js", err, "important"); }
  }
  state.mic.audioCtx = null;
  state.mic.analyser = null;

  if (state.mic.stream) {
    try { state.mic.stream.getTracks().forEach((t) => t.stop()); } catch (err) { globalThis.warnSwallow("features/onboarding/onboarding-mic.js", err, "important"); }
  }
