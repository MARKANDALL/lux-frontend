...
// One-line: CMU dictionary lazy loader + stress inference + DOM upgrade for syllable strips.
...
function upgradePendingStressStrips() {
  ...
  pending.forEach((strip) => {
    ...
    if (!strip.dataset.altIdx) strip.dataset.altIdx = "0";
    ...
    if ((payload.prons || []).length > 1 && !strip.querySelector(".lux-sylAlt")) {
      ...
      btn.textContent = `alts 1/${payload.prons.length}`;
...
/* ------------------------------------------------------------
 * Exports (keep moved code byte-for-byte; export at the end)
 * ---------------------------------------------------------- */

function getCmuDictState() {
  return _CMU_DICT;
}
